        ...
        <div>
          ...
          <div id="order-data-{{ $order->id }}" hidden>
            ...
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:16px;">
              <span style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;padding:3px 8px;
                @if($order->order_status==='Delivered') background:#f0fdf4;color:#15803d;
                @elseif($order->order_status==='Cancelled') background:#fef2f2;color:#dc2626;
                @elseif($order->order_status==='Shipped') background:#eff6ff;color:#1d4ed8;
                @else background:#fffbeb;color:#b45309; @endif">
                {{ $order->order_status }}
              </span>
              <span style="font-size:10px;color:#9ca3af;">{{ $order->created_at->format('d M Y') }}</span>
            </div>

            {{-- Items --}}
            <p style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.1em;color:#9ca3af;margin-bottom:8px;">Items Ordered</p>
            <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:16px;">
              @foreach($order->items as $item)
              <div style="display:flex;align-items:center;gap:12px;background:#f9fafb;border:1px solid #f3f4f6;padding:10px;">
                @if($item->product)
                  <img src="{{ $item->product->image_url }}" alt="{{ $item->product->name }}"
                       style="width:48px;height:48px;object-fit:cover;flex-shrink:0;background:#f3f4f6;">
                @else
                  <div style="width:48px;height:48px;flex-shrink:0;background:#f3f4f6;"></div>
                @endif
                ...
                <p style="font-size:12px;font-weight:700;color:var(--primary);flex-shrink:0;white-space:nowrap;">₹{{ number_format($item->price * $item->quantity, 0) }}</p>
              </div>
              @endforeach
            </div>

            {{-- Totals --}}
            <div style="background:#f9fafb;border:1px solid #f3f4f6;padding:12px;margin-bottom:16px;display:flex;flex-direction:column;gap:6px;">
              <div style="display:flex;justify-content:space-between;align-items:baseline;font-size:11px;color:#9ca3af;">
                <span>Subtotal</span><span>₹{{ number_format($order->subtotal,0) }}</span>
              </div>
              @if($order->discount > 0)
              <div style="display:flex;justify-content:space-between;align-items:baseline;font-size:11px;color:#ef4444;">
                <span>Discount{{ $order->coupon_code ? ' ('.$order->coupon_code.')' : '' }}</span>
                <span style="white-space:nowrap;">−₹{{ number_format($order->discount,0) }}</span>
              </div>
              @endif
              <div style="display:flex;justify-content:space-between;align-items:baseline;font-size:12px;font-weight:700;color:var(--primary);padding-top:6px;border-top:1px solid #f3f4f6;margin-top:2px;">
                <span>Total</span><span style="color:var(--secondary);white-space:nowrap;">₹{{ number_format($order->total,0) }}</span>
              </div>
            </div>
            ...
            <div style="display:flex;gap:8px;">
              <a href="{{ route('account.order.receipt', $order->id) }}" target="_blank"
                 style="flex:1;display:flex;align-items:center;justify-content:center;gap:6px;padding:10px 0;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;border:1px solid #e5e7eb;background:#fff;color:var(--primary);text-decoration:none;">
              ...
              <a href="{{ route('account.order.receipt', $order->id) }}?download=1" target="_blank"
                 style="flex:1;display:flex;align-items:center;justify-content:center;gap:6px;padding:10px 0;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;background:var(--primary);color:#fff;text-decoration:none;">
